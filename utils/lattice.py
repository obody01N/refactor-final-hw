...
def generate_graphite(nx, ny, nz, basis_vectors, basis_atoms):
    positions = np.zeros((nx * ny * nz * len(basis_atoms), 3))  # 初始化位置数组
    atom_index = 0
    boundary = []
        ...
        for j in range(ny):
            for k in range(nz):
                ...
                for l in range(len(basis_frac)):
                    ...
                    pos = basis_frac[l] @ basis_vectors  # 等价
                    xyz = pos + shift
                    positions[atom_index, :3] = xyz
...
def calculate_supercell_vectors(nx, ny, nz, basis_vectors):
